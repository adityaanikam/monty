...
test('datetime input omits timezone metadata without an offset', () => {
  const datetime = {
  ...
  t.deepEqual(encodeValue({ ...datetime, offsetSeconds: null, timezoneName: 'orphaned' }), expected)
  t.deepEqual(encodeValue({ ...datetime, timezoneName: 'orphaned' }), expected)
})
