...
use monty::{
    CompileOptions, MontyObject, MontyRepl, MontyRun, NameLookupResult, PrintWriter, ReplContinuationMode,
    ReplProgress, ResourceLimits, RunProgress, detect_repl_continuation_mode,
};
...
impl Cli {
    ...
    ///
    /// Returns `Ok(None)` when no resource flags were provided, so the caller
    /// uses Monty's finite defaults.
...
/// Dispatches script execution with configured or default finite limits.
fn dispatch_script(
    file_path: &str,
    code: String,
...
/// Dispatches REPL startup with configured or default finite limits.
fn dispatch_repl(
    file_path: &str,
    code: &str,
    limits: Option<ResourceLimits>,
    ...
    run_repl(file_path, code, limits.unwrap_or_default(), mount_table)
}
...
fn run_script(
    ...
    if EXT_FUNCTIONS || mount_table.is_some() {
        let start = Instant::now();
        let progress = match runner.start(inputs, limits, PrintWriter::Stdout) {
            Ok(p) => p,
            Err(err) => {
                let elapsed = start.elapsed();
                eprintln!(
        ...
        let start = Instant::now();
        let value = match runner.run(inputs, limits, PrintWriter::Stdout) {
            Ok(p) => p,
            Err(err) => {
                let elapsed = start.elapsed();
                eprintln!(
...
fn run_repl(file_path: &str, code: &str, limits: ResourceLimits, mut mount_table: Option<MountTable>) -> ExitCode {
    let mut repl = Some(MontyRepl::new(file_path, limits, CompileOptions::default()));
