...
use crate::{
    ...
    heap::{ContainsHeap, DropWithContext, HeapReader},
    intern::StringId,
...
impl ArgValues {
    ...
    pub fn check_zero_args(self, name: &str, heap: &mut HeapReader<'_>) -> RunResult<()> {
        match self {
            Self::Empty => Ok(()),
            other => {
    ...
    ///
    /// On error, properly drops all contained values to maintain reference counts.
    pub fn get_one_arg(self, name: &str, heap: &mut HeapReader<'_>) -> RunResult<Value> {
        match self {
            Self::One(a) => Ok(a),
            other => {
                let count = other.count();
    ...
    ///
    /// On error, properly drops all contained values to maintain reference counts.
    pub fn get_two_args(self, name: &str, heap: &mut HeapReader<'_>) -> RunResult<(Value, Value)> {
        match self {
    ...
    ///
    /// On error, properly drops all contained values to maintain reference counts.
    pub fn get_one_two_args(self, name: &str, heap: &mut HeapReader<'_>) -> RunResult<(Value, Option<Value>)> {
        match self {
    ...
    ///
    /// On error, properly drops all contained values to maintain reference counts.
    pub fn get_zero_one_arg(self, name: &str, heap: &mut HeapReader<'_>) -> RunResult<Option<Value>> {
        match self {
            Self::Empty => Ok(None),
    ...
    pub fn into_pos_only(self, method_name: &str, heap: &mut HeapReader<'_>) -> RunResult<ArgPosIter> {
        match self {
    ...
    #[cold]
    fn unexpected_kwargs_error(kwargs: KwargsValues, method_name: &str, heap: &mut HeapReader<'_>) -> RunError {
