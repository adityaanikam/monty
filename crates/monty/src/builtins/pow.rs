...
fn int_pow_int(b: i64, e: i64, heap: &mut HeapReader<'_>) -> RunResult<Value> {
    if e < 0 {
...
fn int_pow_longint(b: i64, e: &BigInt, heap: &HeapReader<'_>) -> RunResult<Value> {
...
fn longint_pow_int(b: &BigInt, e: i64, heap: &HeapReader<'_>) -> RunResult<Value> {
    if b.is_zero() && e < 0 {
        return Err(ExcType::zero_negative_power());
    }
    if e < 0 {
...
fn longint_pow_longint(b: &BigInt, e: &BigInt, heap: &HeapReader<'_>) -> RunResult<Value> {
