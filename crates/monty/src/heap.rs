...
use crate::{
    ...
    resource::{ResourceError, ResourceLimits, ResourceTracker},
    types::{
...
///
/// Resource accounting is mandatory and serialized with snapshots. Restore paths
/// replace serialized policy and recompute counters before execution resumes.
#[derive(Debug)]
...
impl Heap {
    ...
    /// Applies host policy and recomputes accounting for a restored heap.
    pub(crate) fn restore_tracker(&mut self, limits: ResourceLimits) -> Result<(), ResourceError> {
        let mut allocation_count = 0usize;
        let mut current_memory = 0usize;
        for (id, _) in self.entries.iter().skip(1) {
            allocation_count = allocation_count
                .checked_add(1)
                .ok_or(ResourceError::AllocationUnrepresentable {
                    limit: limits.max_allocations,
                })?;
            current_memory = current_memory.checked_add(self.get(id).py_estimate_size()).ok_or(
                ResourceError::MemoryUnrepresentable {
                    limit: limits.max_memory,
                },
            )?;
        }
        let gc_interval = limits.gc_interval;
        self.tracker = ResourceTracker::restored(limits, allocation_count, current_memory)?;
        self.allocations_since_gc
            .set(self.allocations_since_gc.get().min(gc_interval));
        Ok(())
    }

    /// Checks whether the configured time limit has been exceeded.
    ///
    /// Delegates to the heap's concrete resource tracker.
    ///
    ...
    pub fn get_allocations_since_gc(&self) -> usize {
    ...
    pub fn allocate(&self, data: HeapData) -> Result<HeapId, ResourceError> {
        ...
        if data.is_gc_tracked() {
            self.allocations_since_gc
                .set(self.allocations_since_gc.get().saturating_add(1));
        }
    ...
    pub fn should_gc(&self) -> bool {
        ...
        let interval = self.tracker.gc_interval();
        self.allocations_since_gc.get() >= interval
    }
