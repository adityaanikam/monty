...
pub(crate) struct HeapReader<'a> {
    ...
    pub(crate) tracker: &'a ResourceTracker,
    /// Makes the lifetime `'a` invariant.
    phantom: PhantomData<fn(&'a ResourceTracker) -> &'a ResourceTracker>,
}
...
impl<'a> HeapReader<'a> {
    ...
    #[inline]
    pub fn tracker(&self) -> &'a ResourceTracker {
        self.tracker
    }
    ...
    /// Returns the cached external function for `name`.
    pub fn get_ext_function(&mut self, name: &str) -> Result<Value, ResourceError> {
        self.heap.get_ext_function(self.tracker, name)
    }
    ...
    pub fn dec_ref(&mut self, id: HeapId) {
        ...
        loop {
            ...
            if heap_entry.refcount.get() > 1 {
                ...
                let ext_function_name = match ptr.data(self) {
                    HeapData::ExtFunction(function) => Some(function.cache_key()),
                    _ => None,
                };
                if let Some(name) = ext_function_name {
                    Heap::remove_ext_function_cache_entry(&mut self.heap.ext_function_cache, &name, current_id);
                }
...
#[cfg(any(test, heap_reader_compile_fail_tests))]
pub(crate) fn test_tracker() -> ResourceTracker {
...
impl ContainsHeap for Heap {
    ...
    fn dec_ref(&mut self, id: HeapId) {
        Self::dec_ref(self, &test_tracker(), id);
    }
}
...
impl ContainsHeap for HeapReader<'_> {
    ...
    fn dec_ref(&mut self, id: HeapId) {
        self.heap.dec_ref(self.tracker, id);
    }
}
...
impl Heap {
    ...
    pub fn allocate(&self, tracker: &ResourceTracker, data: HeapData) -> Result<HeapId, ResourceError> {
        tracker.on_grow(|| data.py_estimate_size())?;
    ...
    pub fn get_ext_function(&mut self, tracker: &ResourceTracker, name: &str) -> Result<Value, ResourceError> {
        if let Some(id) = self.ext_function_cache.get(name).copied() {
            ...
            let id = self.allocate(tracker, HeapData::ExtFunction(function))?;
    ...
    pub fn get_timezone_utc(&mut self, tracker: &ResourceTracker) -> Result<Value, ResourceError> {
    ...
    pub fn dec_ref(&mut self, tracker: &ResourceTracker, id: HeapId) {
    ...
    #[inline]
    pub fn should_gc(&self, tracker: &ResourceTracker) -> bool {
        #[cfg(feature = "test-hooks")]
        if self.gc_disabled {
            return false;
        }
    ...
    fn collect_cycles(&mut self) -> usize {
        HeapReader::with(self, &test_tracker(), &mut (), |reader, ()| reader.collect_cycles())
    }

    fn collect_white(&mut self, tracker: &ResourceTracker, roots: Vec<HeapId>) -> usize {
...
mod tests {
    ...
    fn alloc_self_cycle(heap: &Heap) -> HeapId {
        let id = heap
            .allocate(&test_tracker(), HeapData::List(List::new(vec![])))
            .unwrap();
        let entry = heap
            .entries
            .iter()
    ...
    fn cstack_only_cycle_survives_collection() {
        ...
        heap.dec_ref(&test_tracker(), id); // rc = 2, flagged Purple
        ...
        heap.dec_ref(&test_tracker(), id); // rc = 1, re-flagged Purple
        assert_eq!(heap.purple_count, 1);
        heap.collect_cycles();
        assert_eq!(heap.purple_count, 0);
    ...
    fn heap_read_rooted_cycle_survives_collection() {
        ...
        heap.dec_ref(&test_tracker(), id); // rc = 1, flagged Purple
        assert_eq!(heap.purple_count, 1);
    ...
    fn dec_ref_must_not_invalidate_live_heap_read() {
        ...
        HeapReader::with(&mut heap, &test_tracker(), &mut (), |heap, ()| {
    ...
    fn empty_tuple_singleton_survives_collection() {
        ...
        heap.dec_ref(&test_tracker(), EMPTY_TUPLE_ID);
        heap.collect_cycles();
        assert!(
    ...
    fn pending_purple_cycle_round_trips_through_serde() {
        ...
        heap.dec_ref(&test_tracker(), id); // rc 2 → 1
        assert_eq!(heap.purple_count, 1);
    ...
    fn isolated_cycle_with_duplicate_child_refs_is_collected() {
        ...
        heap.dec_ref(&test_tracker(), p_id); // P.rc 2 → 1, flagged Purple
        heap.dec_ref(&test_tracker(), a_id); // A.rc 4 → 3, flagged Purple
    ...
    fn cstack_rooted_cycle_with_duplicate_child_refs_collects_after_pin_dropped() {
        ...
        heap.dec_ref(&test_tracker(), p_id); // P.rc = 2, flagged Purple
        heap.dec_ref(&test_tracker(), a_id); // A.rc = 3, flagged Purple
        ...
        heap.dec_ref(&test_tracker(), p_id);
        heap.collect_cycles();
    ...
    fn callable_iterator_traces_callable_and_sentinel() {
        ...
        let iter = heap
            .allocate(
                &test_tracker(),
                HeapData::CallableIterator(CallableIterator::new(Value::Ref(c), Value::Ref(s))),
            )
            .unwrap();
        let mut traced = vec![];
        ...
        let shared = heap
            .allocate(
                &test_tracker(),
                HeapData::CallableIterator(CallableIterator::new(Value::Ref(c), Value::Ref(c))),
            )
            .unwrap();
        let mut dup = vec![];
    ...
    fn callable_iterator_cycle_is_collected() {
        ...
        let iter = heap
            .allocate(
                &test_tracker(),
                HeapData::CallableIterator(CallableIterator::new(Value::Ref(list), Value::Ref(sentinel))),
            )
            .unwrap();

        // Close the cycle: the callable list references the iterator back.
        heap.inc_ref(iter);
        HeapReader::with(&mut heap, &test_tracker(), &mut (), |reader, ()| {
        ...
        heap.dec_ref(&test_tracker(), list);
        heap.dec_ref(&test_tracker(), iter);
        heap.dec_ref(&test_tracker(), sentinel);
