...
impl<'h, T: ResourceTracker> ContainsHeap<'h> for HeapReader<'h, T> {
    type Tracker = T;
...
impl Heap {
    ...
    #[cfg(test)]
    pub fn dec_ref(&mut self, tracker: &impl ResourceTracker, id: HeapId) {
...
mod tests {
    ...
    fn callable_iterator_traces_callable_and_sentinel() {
        ...
        let c = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
        let s = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
    ...
    fn callable_iterator_cycle_is_collected() {
        let mut heap = Heap::new(16);
        let sentinel = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
        let list = heap.allocate(TRACKER, HeapData::List(List::new(vec![]))).unwrap();
        heap.inc_ref(list);
        ...
        heap.inc_ref(iter);
        HeapReader::with(&mut heap, TRACKER, &mut (), |reader, ()| {
        ...
        heap.dec_ref(TRACKER, list);
        heap.dec_ref(TRACKER, iter);
        heap.dec_ref(TRACKER, sentinel);
