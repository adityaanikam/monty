...
/// Gives cleanup contexts access to heap storage and tracker-aware mutation.
pub(crate) trait ContainsHeap {
    /// Returns the underlying heap for immutable operations.
    fn heap(&self) -> &Heap;

    /// Decrements a reference using the context's resource tracker.
