...
pub struct MontyRepl {
    ...
    heap: Heap,
    /// Resource limits and accounting persisted across snippets.
...
impl Drop for MontyRepl {
    fn drop(&mut self) {
        HeapReader::with(&mut self.heap, &self.tracker, &mut self.globals, |reader, globals| {
            globals.drain(..).drop_with(reader);
        });
    }
}
