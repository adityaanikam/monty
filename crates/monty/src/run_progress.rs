...
///
/// Serialized progress contains accounting state, but loading always applies
/// host-supplied limits and starts a fresh accounting epoch.
...
impl RunProgress {
    /// Deserializes same-version execution state under host-supplied resource limits.
    ///
    /// Limits and counters in the payload are untrusted; live accounting is
    /// recomputed as a fresh epoch before the state is returned.
    pub fn load(bytes: &[u8], limits: ResourceLimits) -> Result<Self, postcard::Error> {
        let mut progress: Self = postcard::from_bytes(bytes)?;
        let heap = match &mut progress {
            Self::FunctionCall(call) => Some(&mut call.snapshot.heap),
            Self::OsCall(call) => Some(&mut call.snapshot.heap),
            Self::ResolveFutures(state) => Some(&mut state.heap),
            Self::NameLookup(lookup) => Some(&mut lookup.snapshot.heap),
            Self::Complete(_) => None,
        };
        if let Some(heap) = heap {
            heap.restore_tracker(limits)
...
impl ResolveFutures {
    ...
    /// Returns resource accounting for the suspended execution.
    #[must_use]
    pub fn tracker(&self) -> &ResourceTracker {
        self.heap.tracker()
    }
