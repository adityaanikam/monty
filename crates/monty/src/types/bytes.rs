...
fn bytes_replace_all(bytes: &[u8], old: &[u8], new: &[u8], heap: &HeapReader<'_>) -> Result<Vec<u8>, ResourceError> {
    if old.is_empty() {
...
fn allocate_bytes(bytes: Vec<u8>, heap: &HeapReader<'_>) -> RunResult<Value> {
