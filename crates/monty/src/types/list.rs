...
fn list_copy(list: &List, heap: &HeapReader<'_>) -> Result<Value, ResourceError> {
...
#[cfg(test)]
mod tests {
    use monty_types::{AssertMessageAnnotations, PrintWriter};
    use num_bigint::BigInt;

    use super::*;
    use crate::{
        heap::{Heap, HeapReader, test_tracker},
    ...
    fn create_heap_with_list_and_longint(list_items: Vec<Value>, index_value: BigInt) -> (Heap, HeapId, HeapId) {
        ...
        let list_id = heap.allocate(&test_tracker(), HeapData::List(list)).unwrap();
        let long_int = LongInt::new(index_value);
        let index_id = heap.allocate(&test_tracker(), HeapData::LongInt(long_int)).unwrap();
    ...
    fn py_setitem_longint_fits_in_i64() {
        ...
        let result = HeapReader::with(&mut heap, &test_tracker(), &mut interns, |reader, interns| {
            let mut vm = VM::new(
                Vec::new(),
                reader,
                interns,
