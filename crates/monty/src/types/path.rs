...
use crate::{
    ...
    hash::HashValue,
    heap::{DropWithContext, Heap, HeapData, HeapId, HeapItem, HeapRead, HeapReadOutput, HeapReader},
...
pub(crate) fn path_div(lhs: &Value, rhs: &Value, heap: &HeapReader<'_>, interns: &Interns) -> RunResult<Option<Value>> {
...
impl Path {
    ...
    pub(crate) fn getattr_by_static(&self, ss: StaticStrings, heap: &HeapReader<'_>) -> RunResult<Option<Value>> {
        let v = match ss {
