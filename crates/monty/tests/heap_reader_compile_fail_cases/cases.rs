...
/// Must not compile: mutating the heap while holding a reference derived from `HeapRead::get`.
///
/// `a.get(heap)` borrows `heap` immutably, and the resulting slice keeps that borrow alive.
/// `dec_ref` requires mutable access to the reader, which conflicts with the live borrow.
///
...
fn heap_mutation_while_reading(list_id: HeapId, heap: &mut Heap) {
    HeapReader::with(heap, &crate::heap::test_tracker(), &mut (), |heap, ()| {
...
fn smuggle_and_swap_reader(heap_a: &mut Heap, heap_b: &mut Heap) {
    HeapReader::with(heap_a, &crate::heap::test_tracker(), &mut (), |reader_a, ()| {
        HeapReader::with(heap_b, &crate::heap::test_tracker(), reader_a, |reader_b, smuggled| {
