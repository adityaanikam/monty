...
///
/// These tests verify that the `ResourceLimits` system correctly enforces
...
use monty::{
    CompileOptions, ExcType, MontyObject, MontyRepl, MontyRun, NameLookupResult, PrintWriter, ResourceLimits,
    RunProgress,
};
...
#[test]
fn resource_limit_defaults_are_finite_and_match_new() {
    let limits = ResourceLimits::default();

    assert_eq!(limits, ResourceLimits::new());
    assert!(limits.max_allocations > 0);
    assert!(!limits.max_duration.is_zero());
    assert!(limits.max_memory > 0);
    assert!(limits.gc_interval > 0);
    assert!(limits.max_recursion_depth > 0);
}

#[test]
fn zero_duration_expires_on_first_execution_check() {
    let run = MontyRun::new("1".to_owned(), "test.py", vec![], CompileOptions::default()).unwrap();
    let error = run
        .run(
            vec![],
            ResourceLimits::default().max_duration(Duration::ZERO),
            PrintWriter::Stdout,
        )
        .unwrap_err();

    assert_eq!(error.exc_type(), ExcType::TimeoutError);
}

#[test]
fn zero_recursion_rejects_the_first_function_frame() {
    let code = "def f():\n    return 1\nf()";
    let run = MontyRun::new(code.to_owned(), "test.py", vec![], CompileOptions::default()).unwrap();
    let error = run
        .run(
            vec![],
            ResourceLimits::default().max_recursion_depth(0),
            PrintWriter::Stdout,
        )
        .unwrap_err();
...
#[test]
fn representational_maxima_are_finite_usable_limits() {
    let limits = ResourceLimits::default()
        .max_allocations(usize::MAX)
        .max_duration(Duration::MAX)
        .max_memory(usize::MAX)
        .gc_interval(usize::MAX)
        .max_recursion_depth(usize::MAX);
    let run = MontyRun::new(
        "sum(range(100))".to_owned(),
        "test.py",
        vec![],
    ...
    assert_eq!(
        run.run(vec![], limits, PrintWriter::Stdout).unwrap(),
        MontyObject::Int(4950)
    );
}

#[test]
fn resource_limit_builders_only_override_the_selected_limit() {
    let defaults = ResourceLimits::default();

    assert_eq!(
        ResourceLimits::default().max_allocations(7),
        ResourceLimits {
            max_allocations: 7,
            ..defaults
        }
    ...
    assert_eq!(
        ResourceLimits::default().max_duration(Duration::from_millis(7)),
        ResourceLimits {
    ...
    assert_eq!(
        ResourceLimits::default().max_memory(7),
        ResourceLimits {
            max_memory: 7,
            ..defaults
        }
    ...
    assert_eq!(
        ResourceLimits::default().gc_interval(7),
        ResourceLimits {
            gc_interval: 7,
            ..defaults
        }
    ...
    assert_eq!(
        ResourceLimits::default().max_recursion_depth(7),
        ResourceLimits {
...
fn compact_id_does_not_allocate() {
    ...
    let result = run.run(vec![], limits, PrintWriter::Stdout).unwrap();
...
fn wide_id_respects_allocation_limit() {
    ...
    let limits = ResourceLimits::new().max_allocations(0);
    let exc = run.run(vec![], limits, PrintWriter::Stdout).unwrap_err();
...
#[test]
fn zero_memory_and_allocation_limits_reject_the_first_charge() {
    for limits in [
        ResourceLimits::default().max_memory(0),
        ResourceLimits::default().max_allocations(0),
    ] {
        let run = MontyRun::new("[]".to_owned(), "test.py", vec![], CompileOptions::default()).unwrap();
        let error = run.run(vec![], limits, PrintWriter::Stdout).unwrap_err();
...
#[test]
fn run_with_default_limits_succeeds() {
    // Verify that ordinary execution fits within the finite defaults.
    let code = r"
result = []
for i in range(100):
    result.append(str(i))
...
";
    ...
    let output = ex
        .run_ref_counts_with_limits(vec![], limits)
...
#[test]
#[cfg(feature = "ref-count-return")]
fn zero_gc_interval_collects_at_every_eligible_allocation() {
    let code = r"
for i in range(3):
    a = []
    a.append(a)
...
";
    let run = MontyRun::new(code.to_owned(), "test.py", vec![], CompileOptions::default()).unwrap();
    let output = run
        .run_ref_counts_with_limits(vec![], ResourceLimits::default().gc_interval(0))
        .unwrap();

    assert_eq!(output.allocations_since_gc, 0);
}
...
fn executor_iter_resource_limit_on_resume() {
    ...
    let limits = ResourceLimits::new().max_allocations(5);
    let progress = run.start(vec![], limits, PrintWriter::Stdout).unwrap();
...
fn executor_iter_resource_limit_before_function_call() {
    ...
    let result = run.start(vec![], limits, PrintWriter::Stdout);
...
///
/// Function call namespaces (local variables) are tracked by ResourceLimits.
...
fn bigint_edge_cases_always_succeed() {
    ...
    let code = "0 ** 10000000";
    let ex = MontyRun::new(code.to_owned(), "test.py", vec![], CompileOptions::default()).unwrap();
    let result = ex.run(vec![], limits.clone(), PrintWriter::Stdout);
...
// built with the generous default time limit, then execution pauses at `interrupt()`. A short time
...
fn call_function_enforces_max_duration() {
    let limits = ResourceLimits::new().max_duration(Duration::from_millis(50));
    let mut repl = MontyRepl::new("test.py", limits, CompileOptions::default());
    repl.feed_run(
        "def spin():\n    while True:\n        pass",
        vec![],
        PrintWriter::Stdout,
...
/// Helper: builds a large object under the default limit, then runs `repr()` on it
...
fn assert_repr_timeout(code: &str, label: &str) {
    ...
    // Phase 1: build the large object with the default time limit
    let limits = ResourceLimits::new();
    let progress = run.start(vec![], limits, PrintWriter::Stdout).unwrap();
